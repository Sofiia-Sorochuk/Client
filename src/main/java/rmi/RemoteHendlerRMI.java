...
public class RemoteHendlerRMI implements Closeable {
    ...
    private void commandCheker(String textFromLable) {
                ...
                try {
                    switch (command) {
                        case CMD_PING:
                            ...
                            break;
                        case CMD_ECHO:
                            responseInfo = proxy.echo(protocolManager.parserCommandToGetSinglePostArgument(textFromLable));
                            isRegex = true;
                            break;
                        case CMD_PROCESS:
        ...
        if (!isRegex) {
            responseInfo = "Wrong input command!";
        }
        isRegex = false;
    }
    ...
    private void process(String stringPathRandFile, String stringPathSortFile) {
        ...
        Compute.Sort sort = null;
        try {
            sort = new Compute.Sort(fileSortNum.getName(), fileRandomNum);
        ...
        try {
            long timeProcesMethod = proxy.timeProcesMethod(sort);
            responseInfo = "Done! Time proces algoritm QuickSort = " + timeProcesMethod + " nsec| " + timeProcesMethod / 1000 + " mksec| " + timeProcesMethod / 1000000 + " msec.";
        } catch (RemoteException e) {
            responseInfo = "There was an server error processing!";
        }
    }
...
//                    e.printStackTrace();
                    responseInfo = "Server droped down!";
                    close();
                }
            }
        };
